...
@pytest.mark.parametrize(
    "timeout, accepted, result_predicate",
    [
...
@pytest.mark.asyncio
async def test_eventually_with_timeout(timeout, accepted, result_predicate):
    ...
    assert assertion.accepted is accepted
    assert result_predicate(assertion.result)


@pytest.mark.parametrize("timeout, accepted", [(1.0, True), (0.1, False)])
@pytest.mark.asyncio
async def test_while_eventually(timeout, accepted):
    ...
    assert assertion.done
    assert assertion.accepted == accepted
